...
class MBAgent(BaseAgent):
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        for i in range(self.ensemble_size):
            ...
            # TODO is this along the right dimension?
            batch = np.array([1] * num_data_per_ens + [0] * (num_data - num_data_per_ens), dtype=bool)
            np.random.shuffle(batch)
            #print("batch.size", batch.shape)
            
            observations = ob_no[batch]
            actions = ac_na[batch]
            next_observations = next_ob_no[batch]
